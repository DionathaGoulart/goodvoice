...
/// Whether the target links with MSVC rather than a GNU-flavoured toolchain.
///
/// Read from the target rather than `cfg!`, which in a build script describes
/// the host. goodvoice vendor patch; see DR-11 in .harness/plan.md.
...
mod webrtc {
    ...
    // Mirrored by a cargo:rerun-if-changed in main(); keep the two in step.
    ...
    pub(super) fn build_if_necessary() -> Result<()> {
        ...
        // Upstream shells out to `cp -a`, which Windows does not have. fs_extra is
        // already a build dependency and `content_only` is the trailing-dot trick:
        // it prevents `webrtc-audio-processing/webrtc-audio-processing` nesting on a
        // 2nd invocation. goodvoice vendor patch 1 of 5; see DR-11.
        std::fs::create_dir_all(&webrtc_source_dir)
            .context("creating the webrtc source directory")?;
        let copy_options =
        ...
        fs_extra::dir::copy(bundled_source_path, &webrtc_source_dir, &copy_options)
            .context("copying the bundled webrtc source")?;
        ...
        if target_is_msvc() {
            // meson.build asks for c++17. One file — agc2/input_volume_stats_reporter.cc
            // — uses designated initializers, which GCC and Clang accept in C++17 as an
            // extension and MSVC accepts only from C++20. goodvoice vendor patch 3 of 5;
            // see DR-11.
    ...
    fn get_defined_symbols(archive_path: &std::path::Path) -> Result<Vec<String>> {
        // GNU spelling upstream; Windows has no `nm`, and the llvm-tools component
        // that already supplies rust-objcopy supplies rust-nm, which answers
        // identically. goodvoice vendor patch 2 of 5; see DR-11.
...
fn main() -> Result<()> {
    ...
    if target_is_msvc() {
        // cl.exe reads `-Wno-unused-parameter` as /W followed by something that is
        // not a number, and wants its standard in its own spelling. c++20 rather
        // than c++17 to match what the library above was built with.
        // goodvoice vendor patch 4 of 5; see DR-11.
    ...
    // ...and the bundled tree, which is stated for the same reason and was not.
    // It is copied into OUT_DIR and built there, so an edit to it is invisible
    // unless cargo is told to watch it. goodvoice vendor patch; see DR-24.
    #[cfg(feature = "bundled")]
    ...
    // MSVC writes `<name>.lib` where a GNU toolchain writes `lib<name>.a`. Getting
    // this wrong skips the wrapper's symbol prefixing silently, and the link then
    // fails to resolve. goodvoice vendor patch 5 of 5; see DR-11.
    let wrapper_lib = out_dir().join(if target_is_msvc() {
...
/// The `nm` to list an archive's symbols with.
///
/// Upstream runs bare `nm`, which is the GNU spelling and does not exist on
/// Windows. Rather than pick one replacement, try the ones this build could
/// plausibly have, in order of how much it already depends on them: rustup's
/// llvm-tools if that component is installed, then `llvm-nm` — bindgen already
/// requires LLVM on Windows, so it is the one extra tool a Windows build is
/// certain to have — and finally the original. All of them read a COFF archive
/// and answer `--format=posix` identically.
/// goodvoice vendor patch 2 of 5; see DR-11.
fn determine_nm_path() -> Result<PathBuf> {
    let mut candidates: Vec<PathBuf> = Vec::new();
    if let Ok(bin) = toolchain_bin_dir() {
        candidates.push(bin.join("rust-nm"));
    }
    if target_is_msvc() {
        if let Ok(llvm) = env::var("LIBCLANG_PATH") {
            candidates.push(PathBuf::from(llvm).join("llvm-nm"));
        }
        candidates.push(PathBuf::from("llvm-nm"));
    }
    candidates.push(PathBuf::from("nm"));

    for candidate in &candidates {
        // Running it is the only honest test: a bare name has to be resolved
        // through PATH, and a path on Windows may or may not want `.exe`.
    ...
    bail!(
        "no usable nm found (tried {candidates:?}). Install the llvm-tools rustup \
         component, or put llvm-nm on PATH."
    )
}

/// `<sysroot>/lib/rustlib/<HOST_TRIPLE>/bin`, where the llvm-tools live.
fn toolchain_bin_dir() -> Result<PathBuf> {
    ...
    // HOST, not TARGET: this is where the tools that are *running* live.
...
fn determine_objcopy_path() -> Result<PathBuf> {
    let objcopy = toolchain_bin_dir()?.join("rust-objcopy");

    // Optional: verification. `.exe` on Windows, where the bare name never exists.
    if !objcopy.exists() && !objcopy.with_extension("exe").exists() {
